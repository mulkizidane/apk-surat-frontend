...
import { getSuratUntukDisposisi } from '../api/api'; // Anda perlu membuat fungsi API ini

export default function DisposisiPimpinanPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await getSuratUntukDisposisi();
        setSuratList(response.data || []);
      // eslint-disable-next-line no-unused-vars
      } catch (err) {
        setError('Gagal memuat data surat untuk disposisi.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Disposisi Surat Masuk</h1>
      <div className="bg-white p-8 rounded-lg shadow-md">
        {suratList.length === 0 ? (
          <p className="text-gray-500">Tidak ada surat baru yang perlu didisposisi.</p>
        ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {suratList.map((surat) => (
                  <tr key={surat.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{surat.nama_pengirim_eksternal || 'Internal'}</td>
